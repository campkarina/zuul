...
public class ThreeDView extends View {
  ...
  private void handleCoordinates(int x, int y) {
    ...
    if (p.getCurrentRoom().hasItem(new Item(TREASURE, 100, 0, true))) win();
    ...
    if (doorEast.contains(x, y)) {
      ...
      if ((p.getCurrentRoom().getExit(Direction.EAST) != null)
          && (p.getCurrentRoom().getExit(Direction.EAST).getLocked() != true)) currentMapRoom++;
      if ((p.getCurrentRoom().getExit(Direction.EAST).getLocked() != true)
          || (unlocked == true)) p.doCommand(Command.parse(GO_EAST));
      ...
      if ((p.getCurrentRoom().getExit(Direction.WEST) != null)
          && (p.getCurrentRoom().getExit(Direction.WEST).getLocked() != true)) currentMapRoom--;
      ...
      if ((p.getCurrentRoom().getExit(Direction.NORTH) != null)
          && (p.getCurrentRoom().getExit(Direction.NORTH).getLocked() != true)) currentMapRoom -= 4;
      if ((p.getCurrentRoom().getExit(Direction.NORTH).getLocked() != true)
          || (unlocked == true)) p.doCommand(Command.parse(GO_NORTH));
      else if (!p.getInventory().contains(new Item("Key", true))) JOptionPane
          .showMessageDialog(
              this,
              "The Door is locked!\nYou are sure the treasure is just beyond.\nIf only you had a Key..");
      ...
      if ((p.getCurrentRoom().getExit(Direction.SOUTH) != null)
          && (p.getCurrentRoom().getExit(Direction.SOUTH).getLocked() != true)) currentMapRoom += 4;
      if ((p.getCurrentRoom().getExit(Direction.SOUTH).getLocked() != true)
          || (unlocked == true)) p.doCommand(Command.parse(GO_SOUTH));
      else if (!p.getInventory().contains(new Item("Key", true))) JOptionPane
          .showMessageDialog(
              this,
              "The Door is locked!\nYou are sure the treasure is just beyond.\nIf only you had a Key..");
      ...
      if (!p.getCurrentRoom().getItems().isEmpty()) popup = JOptionPane
          .showOptionDialog(this, "This Room has the following items:",
        ...
        JOptionPane.showOptionDialog(this,
            "This room does not have any items in it!", "Current Room",
  ...
  private void setupView() {
    ...
    JLabel chestLabel;
    if (p.getCurrentRoom().hasItems()) chestLabel = new JLabel(new ImageIcon(
        IMAGES_CHEST_IN_ROOM_FILLED_PNG));
    else chestLabel = new JLabel(new ImageIcon(IMAGES_CHEST_IN_ROOM_PNG));
    ...
    // }
    if (p.getCurrentRoom().hasMonsters()) if (p.getCurrentRoom().getMonster()
        .getName().equals(Game.BOSS2)) {
      JLabel bossLabel = new JLabel(new ImageIcon(IMAGES_BOSS1_IN_ROOM_PNG));
      JLabel bossLabel2 = new JLabel(new ImageIcon(IMAGES_BOSS2_IN_ROOM_PNG));
    ...
    if (p.getCurrentRoom().getExit(Direction.NORTH) != null) {
      ...
      if (p.getCurrentRoom().getExit(Direction.NORTH).getLocked()) northLabel = new JLabel(
          new ImageIcon(IMAGES_NORTH_DOOR_LOCKED_PNG));
      else northLabel = new JLabel(new ImageIcon(IMAGES_NORTH_DOOR_PNG));
    ...
    if (p.getCurrentRoom().getExit(Direction.EAST) != null) {
      JLabel eastLabel;
      eastPanel.removeAll();
      if (p.getCurrentRoom().getExit(Direction.EAST).getLocked()) eastLabel = new JLabel(
          new ImageIcon(IMAGES_EAST_DOOR_LOCKED_PNG));
      else eastLabel = new JLabel(new ImageIcon(IMAGES_EAST_DOOR_PNG));
    ...
    if (p.getCurrentRoom().getExit(Direction.WEST) != null) {
      JLabel westLabel;
      westPanel.removeAll();
      if (p.getCurrentRoom().getExit(Direction.WEST).getLocked()) westLabel = new JLabel(
          new ImageIcon(IMAGES_WEST_DOOR_LOCKED_PNG));
      else westLabel = new JLabel(new ImageIcon(IMAGES_WEST_DOOR_PNG));
    ...
    if (p.getCurrentRoom().getExit(Direction.SOUTH) != null) {
      ...
      if (p.getCurrentRoom().getExit(Direction.SOUTH).getLocked()) southLabel = new JLabel(
          new ImageIcon(IMAGES_SOUTH_DOOR_LOCKED_PNG));
      else southLabel = new JLabel(new ImageIcon(IMAGES_SOUTH_DOOR_PNG));
  ...
  protected void updateHealthField() {
    ...
    if (p.getCurrentRoom().hasMonsters()) s += "\nMonster Health: "
